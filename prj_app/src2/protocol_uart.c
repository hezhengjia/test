...
//{
// MOV R0, #1           //;
// MSR FAULTMASK, R0    //; Ê∏ÖÈô§FAULTMASK Á¶ÅÊ≠¢‰∏ÄÂàá‰∏≠Êñ≠‰∫ßÁîü
// LDR R0, =0xE000ED0C  //;
// LDR R1, =0x05FA0004  //;
// STR R1, [R0]         //; Á≥ªÁªüËΩØ‰ª∂Â§ç‰Ωç
//
//deadloop
//    B deadloop        //; Ê≠ªÂæ™ÁéØ‰ΩøÁ®ãÂ∫èËøêË°å‰∏çÂà∞‰∏ãÈù¢ÁöÑ‰ª£Á†Å
//}
...
//head
    ...
    case 4://lenth
        ...
        else
        {
            ...
            buf[buf_len++]=tmp;
            sta++;
            if(5==sta)//Âà§Êñ≠ÈïøÂ∫¶ÊòØÂê¶ÂêàÊ≥ï
            {
                tmp16=buf[2];
                tmp16<<=8;
                tmp16|=buf[3];
                left=tmp16;
                if(tmp16>(UART_FRAM_LEN))//ÈïøÂ∫¶ÈîôËØØ
                {
                    sta=0;
                    break;
                }
...
void task_protocol_dataout_fun(uint16_t cmd,uint8_t*pdata,uint16_t datalen)
{
    ...
    switch(cmd)
    {
    case SET_NETWORK_PARAMETERS://‰∏ä‰ΩçÊú∫Âô®ÈÖçÁΩÆÁΩëÁªúÂèÇÊï∞
        //0x82+ip(4bytes)+port(2byte)+dhcp(1byte)+Êú¨Êú∫IP+ÁΩëÂÖ≥+Â≠êÁΩëÊé©Á†Å+DNS1+DNS2
        ...
        WriteFlgBootSys();
        break;
    case READ_NETWORK_PARAMETERS://ËØªÂèñÁΩëÁªúÂèÇÊï∞
        //0x82+ip(4bytes)+port(2byte)+dhcp(1byte)+Êú¨Êú∫IP+ÁΩëÂÖ≥+Â≠êÁΩëÊé©Á†Å+DNS1+DNS2
        len = 0;
        ...
        break;
    case SET_FACTORY://ÊÅ¢Â§çÂá∫ÂéÇËÆæÁΩÆ
        buf[0] = 0;
        ...
        WriteFlgBootSys();
        break;
    case UPFILE_START://ÂçáÁ∫ßÂºÄÂßã
    ...
    case UPFILE_PAR://ÂçáÁ∫ßÂèÇÊï∞
        ...
        file_cmd = UPFILE_PAR;
        break;
    case INFORMATION_PAR://ÂºÄÂèëÊùø‰ø°ÊÅØ
...
/*
Õ¯πÿ∑¢ÀÕ≥∆÷ÿ¥´∏–∆˜πÈ¡„÷∏¡Ó0x36
Õ¯πÿ∑¢ÀÕ«Î«ÛÀ«Œπ∆˜¥´∏–∆˜◊¥Ã¨÷∏¡Ó0x37
.Õ¯πÿ∑¢ÀÕ«Î«ÛÀ«Œπ∆˜…Ë±∏–≈œ¢÷∏¡Ó0x39
...
/*
Õ¯πÿ∑¢ÀÕ«Î«ÛÀ«Œπ∆˜ µº µƒΩ¯Œπ÷ÿ¡ø÷∏¡Ó0x38
0x00:«Î«ÛÀ«Œπ∆˜ µº Ω¯¡œ÷ÿ¡ø£¨
0x01:«Î«ÛÀ«Œπ∆˜ µº Œπ¡œ÷ÿ¡ø
